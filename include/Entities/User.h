...
bool is_Valid_DOB(int dat,int month,int year);
...
class User{
    ...
    public:
        ...
        User():firstName(),lastName(),phoneNumber(),homeAddress(),email(),password(){}
        User(string firstName,string lastName, string phoneNumber,string address, int day, int month, int year, string email, string password):firstName(firstName),lastName(lastName),phoneNumber(phoneNumber),homeAddress(address),day(day),month(month),year(year),email(email),password(password){}
        void updateProfile(){
            viewProfile();
            cout<<"Update:"<<endl;
...
User Register(){
    ...
    User newUser(firstName, lastName, phoneNumber,address, day, month, year, email, password);
    return newUser;
}
...
bool is_Valid_DOB(int day,int month,int year) {
    if(year<1910 || year >2026) return false;
